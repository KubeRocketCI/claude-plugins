...
> Template for business process analysis. Populate the `{{variables}}`; the `<instructions>` blocks are internal guidance for generating each section and must never appear in the final document.
